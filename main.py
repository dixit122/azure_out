...
inp = tokenizer("Would one have ever thought that one day, we will be on the moon! It was just like a dream, wasn’t it? But now it seems that it is going to be possible in the near future. Similarly, whatever seems difficult to us today is not necessarily difficult (out of the reach). </s> <2en>", add_special_tokens=False, return_tensors="pt", padding=True).input_ids # tensor([[  466,  1981,    80, 25573, 64001, 64004]])
...
model_output=model.generate(inp, use_cache=True, num_beams=4, max_length=100, min_length=1, early_stopping=True, pad_token_id=pad_id, bos_token_id=bos_id, eos_token_id=eos_id, decoder_start_token_id=tokenizer._convert_token_to_id_with_added_voc("<2en>"))
